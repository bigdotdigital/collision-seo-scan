...
export default async function ReportPage({ params }: { params: { scanId: string } }) {
  ...
  const executiveSummary = (scanRecord.aiSummary || '').trim()
    ...
    const shopRating = typeof reviewGap?.shopRating === 'number' ? reviewGap.shopRating : 0;
    const competitorRating = typeof reviewGap?.competitorRating === 'number' ? reviewGap.competitorRating : 0;
    const competitorRows = [
      {
        name: 'Your Shop',
        score: scoreTotal,
        reviews: `${shopRating.toFixed(1)} ★`
      },
      ...vm.competitors.slice(0, 2).map((comp, idx) => ({
        name: comp.name,
        score: Math.max(48, scoreTotal - (idx + 1) * 8),
        reviews: idx === 0 ? `${competitorRating.toFixed(1)} ★` : 'n/a'
      }))
    ];
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          <article className="rounded-lg border border-slate-200 bg-slate-50 p-4">
            ...
            <p className="mt-1 text-lg font-semibold">
              {shopRating.toFixed(1)} stars • {reviewGap?.shopReviews || 0} reviews
            </p>
          </article>
          <article className="rounded-lg border border-slate-200 bg-slate-50 p-4">
            <p className="text-xs uppercase tracking-wide text-slate-500">Top competitor</p>
            <p className="mt-1 text-lg font-semibold">
              {competitorRating.toFixed(1)} stars • {reviewGap?.competitorReviews || 0} reviews
            </p>
          </article>
          <article className="rounded-lg border border-slate-200 bg-slate-50 p-4">
            <p className="text-xs uppercase tracking-wide text-slate-500">Review gap</p>
            <p className="mt-1 text-lg font-semibold">{reviewGap?.reviewGap || 0} reviews</p>
            <p className="text-sm text-slate-700">Impact: {reviewGap?.impact || 'Visibility gap detected'}</p>
          </article>
        </div>
